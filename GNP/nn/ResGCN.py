...
#-----------------------------------------------------------------------------
# A GCN layer for any matrix A.
class GCNConvAAA(nn.Module):
    ...
    def forward(self, AA, R):                         # R: (n, batch_size, in_dim)
        ...
        if in_dim > self.out_dim:
            ...
            R = AA @ R                          # (n, batch_size * out_dim)
            R = R.view(n, batch_size, self.out_dim)  # (n, batch_size, out_dim)
        else:
            R = R.view(n, batch_size * in_dim)       # (n, batch_size * in_dim)
            R = AA @ R                          # (n, batch_size * in_dim)
            R = R.view(n, batch_size, in_dim)        # (n, batch_size, in_dim)
            R = self.fc(R)                           # (n, batch_size, out_dim)
        return R
...
class ResGCN_AAA(nn.Module):
    
    def __init__(self, num_layers, embed, hidden, drop_rate,
